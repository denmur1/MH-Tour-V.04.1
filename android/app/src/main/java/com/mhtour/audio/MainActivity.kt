...
class MainActivity : ComponentActivity() {
    ...
    private val donationUrl = "https://link.dana.id/minta?full_url=https://qr.dana.id/v1/281012012023022707162488"
    ...
    private fun home() {
        // UI HOME ONLY: server, LiveKit, session flow and donation URL are unchanged.
        ...
        // Donation action opens the in-app Donasi Seikhlasnya page.
    ...
    private fun donationScreen() {
        val root = page(
            "Donasi Seikhlasnya",
            "Dukungan Anda membantu pengembangan MH Tour dan layanan audio untuk jemaah."
        )
        val body = contentColumn()

        val card = card()
        card.gravity = Gravity.CENTER_HORIZONTAL
        card.addView(tv("DONASI DANA", 12f, gold, true))
        addGap(card, 8)
        ...
        card.addView(qr, LinearLayout.LayoutParams(dp(280), dp(280)).apply {
            gravity = Gravity.CENTER_HORIZONTAL
        })
        addGap(card, 12)

        val info = tv(
            "Scan QR Code untuk memberikan donasi seikhlasnya melalui DANA.",
            13f,
            muted
        )
        info.gravity = Gravity.CENTER
        card.addView(info, LinearLayout.LayoutParams(-1, ViewGroup.LayoutParams.WRAP_CONTENT))
        addGap(card, 12)

        val link = tv(donationUrl, 11f, green, false)
        link.gravity = Gravity.CENTER
        link.setOnClickListener { openDonationLink() }
        card.addView(link, LinearLayout.LayoutParams(-1, ViewGroup.LayoutParams.WRAP_CONTENT))
        addGap(card, 14)

        val open = primaryButton("Buka Link Donasi", green)
        card.addView(open, LinearLayout.LayoutParams(-1, dp(52)))
        addGap(card, 8)

        val download = outlineButton("Unduh QR Code")
        card.addView(download, LinearLayout.LayoutParams(-1, dp(50)))
        addGap(card, 8)

        val back = outlineButton("Kembali ke Beranda")
        card.addView(back, LinearLayout.LayoutParams(-1, dp(50)))
        ...
        open.setOnClickListener { openDonationLink() }
        download.setOnClickListener { downloadDonationQr() }
        back.setOnClickListener { home() }
    }

    private fun openDonationLink() {
        try {
            startActivity(Intent(Intent.ACTION_VIEW, Uri.parse(donationUrl)))
        } catch (e: Exception) {
            Toast.makeText(this, "Tidak dapat membuka link donasi.", Toast.LENGTH_LONG).show()
        }
    }

    private fun donationQrBitmap(): Bitmap = BitmapFactory.decodeResource(resources, R.drawable.donation_qr)

    private fun downloadDonationQr() {
        try {
            val bitmap = donationQrBitmap()
            val fileName = "MH_Tour_Donasi_DANA.png"

            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                val values = android.content.ContentValues().apply {
                    put(MediaStore.Images.Media.DISPLAY_NAME, fileName)
                    put(MediaStore.Images.Media.MIME_TYPE, "image/png")
                    put(MediaStore.Images.Media.RELATIVE_PATH, "Pictures/MH Tour")
                ...
                try {
                    contentResolver.openOutputStream(uri)?.use { output ->
                        bitmap.compress(Bitmap.CompressFormat.PNG, 100, output)
                            .also { ok -> if (!ok) error("Gagal menyimpan QR Code") }
                    } ?: error("Tidak dapat membuka file QR Code")
                    values.clear()
                    values.put(MediaStore.Images.Media.IS_PENDING, 0)
                    contentResolver.update(uri, values, null, null)
                    Toast.makeText(this, "QR Code berhasil disimpan di Pictures/MH Tour.", Toast.LENGTH_LONG).show()
                } catch (e: Exception) {
                    contentResolver.delete(uri, null, null)
                    throw e
                }
            } else {
                val dir = File(getExternalFilesDir(android.os.Environment.DIRECTORY_PICTURES), "MH Tour")
                if (!dir.exists() && !dir.mkdirs()) error("Folder penyimpanan tidak dapat dibuat")
                val file = File(dir, fileName)
                FileOutputStream(file).use { output ->
                    if (!bitmap.compress(Bitmap.CompressFormat.PNG, 100, output)) {
                        error("Gagal menyimpan QR Code")
                    }
                }
                Toast.makeText(this, "QR Code berhasil disimpan di folder Pictures/MH Tour aplikasi.", Toast.LENGTH_LONG).show()
            }
        } catch (e: Exception) {
            Toast.makeText(this, "QR Code gagal diunduh: ${e.message ?: "kesalahan penyimpanan"}", Toast.LENGTH_LONG).show()
        }
    }

    private fun shareQrImage(code: String) {
        try {
            val bitmap = makeQrBitmap("MHTOUR|JOIN|$code")
            val file = File(cacheDir, "MH_Tour_$code.png")
            FileOutputStream(file).use { output ->
                if (!bitmap.compress(Bitmap.CompressFormat.PNG, 100, output)) error("Gagal membuat QR")
            }
            val uri = FileProvider.getUriForFile(this, "${BuildConfig.APPLICATION_ID}.fileprovider", file)
            val intent = Intent(Intent.ACTION_SEND).apply {
                type = "image/png"
                putExtra(Intent.EXTRA_STREAM, uri)
                putExtra(Intent.EXTRA_TEXT, "MH Tour — Rombongan: $code")
                addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)
            }
            startActivity(Intent.createChooser(intent, "Bagikan QR Rombongan MH Tour"))
        } catch (e: Exception) {
            Toast.makeText(this, "QR tidak dapat dibagikan: ${e.message}", Toast.LENGTH_LONG).show()
        }
    }
